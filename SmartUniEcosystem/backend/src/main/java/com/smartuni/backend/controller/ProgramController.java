...
import com.smartuni.backend.repository.ProgramRepository;
...
public class ProgramController {
    ...
    @GetMapping
    public List<Program> get(Program probe) {
    ...
    @PutMapping("/{id}")
    public Program update(@PathVariable String id, @RequestBody Program entity) {
        entity.setId(id);
        return repository.save(entity);
    }

    @PatchMapping("/{id}")
    public Program patch(@PathVariable String id, @RequestBody Map<String, Object> updates) {
        Program entity = repository.findById(id).orElse(null);
        if (entity == null) return null;
        
        org.springframework.util.ReflectionUtils.doWithFields(Program.class, field -> {
