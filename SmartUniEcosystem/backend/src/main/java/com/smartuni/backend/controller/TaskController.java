...
import com.smartuni.backend.entity.Task;
import com.smartuni.backend.repository.TaskRepository;
...
public class TaskController {
    ...
    @GetMapping
    public List<Task> get(Task probe) {
    ...
    @PutMapping("/{id}")
    public Task update(@PathVariable String id, @RequestBody Task entity) {
        entity.setId(id);
        return repository.save(entity);
    }

    @PatchMapping("/{id}")
    public Task patch(@PathVariable String id, @RequestBody Map<String, Object> updates) {
        Task entity = repository.findById(id).orElse(null);
        if (entity == null) return null;
        
        org.springframework.util.ReflectionUtils.doWithFields(Task.class, field -> {
