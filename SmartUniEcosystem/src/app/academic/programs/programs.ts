      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-slate-500 mt-1">
            Manage university degree programs, curriculum, and credit structures.
          </p>
        </div>
        <button
          (click)="openModal()"
          class="px-5 py-2.5 bg-indigo-600 text-white rounded-xl shadow-lg shadow-indigo-500/30 hover:-translate-y-0.5 transition-all font-bold flex items-center gap-2"
        >
          <svg
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="glass-panel px-4 py-2 flex items-center gap-3">
          ...
          <input
            ...
            class="bg-transparent border-none outline-none text-sm w-full font-medium"
          />
        </div>
        <select
          [(ngModel)]="selectedDept"
          (change)="onFilter()"
          class="glass-panel px-4 py-2 text-sm font-bold outline-none cursor-pointer"
        >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div
          *ngFor="let prog of filteredPrograms()"
          class="glass-card p-6 border-l-4 border-l-indigo-500 hover:-translate-y-1 transition-all group"
        >
          <div class="flex justify-between items-start mb-4">
            ...
            <div class="flex gap-1">
              <button
                (click)="openModal(prog); $event.stopPropagation()"
          ...
          <p class="text-xs font-bold text-indigo-600 dark:text-indigo-400 mb-6">
    ...
    >
      <div class="glass-panel max-w-lg w-full p-6 shadow-2xl">
        <h2 class="text-xl font-bold text-slate-900 dark:text-white mb-4">
          {{ editingProgramId() ? 'Edit Academic Program' : 'Add New Program' }}
        </h2>
        <form [formGroup]="progForm" (ngSubmit)="saveProgram()" class="space-y-4">
          <div>
            ...
            <input
              ...
              type="text"
              class="w-full px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-indigo-500 text-sm"
            />
          </div>
          <div>
          ...
          <div class="flex gap-3 mt-6">
            <button
              ...
              class="flex-1 py-2 text-slate-500 font-bold hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl"
            >
              Cancel
            </button>
            <button
              type="submit"
              [disabled]="progForm.invalid"
              class="flex-1 py-2 bg-indigo-600 text-white font-bold rounded-xl shadow-lg"
            >
