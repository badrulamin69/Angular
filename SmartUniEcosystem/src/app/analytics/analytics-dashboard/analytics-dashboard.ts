  ...
  template: `
    <div class="space-y-6 animate-fade-in-up">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <button (click)="exportData()" class="px-4 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all font-bold flex items-center gap-2">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8 animate-fade-in delay-200">
        <div class="glass-panel p-6">
          ...
          <div class="h-64 flex items-end justify-between gap-4 border-b border-l border-slate-200 dark:border-slate-800 pb-2 pl-2">
            <div *ngFor="let bar of growthTrend()" class="w-full transition-all duration-500 rounded-t-sm relative group"
                 [ngClass]="bar.current ? 'bg-mit-red hover:bg-primary-600' : 'bg-indigo-500 hover:bg-indigo-400'"
                 [style.height.%]="bar.percentage">
              <div class="opacity-0 group-hover:opacity-100 absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-xs px-2 py-1 rounded whitespace-nowrap z-10 transition-opacity">
                {{ bar.label }}: {{ bar.val }}
              </div>
              <span class="absolute -bottom-6 left-1/2 -translate-x-1/2 text-xs font-bold text-slate-400">{{ bar.label }}</span>
            </div>
          </div>
        </div>

        <div class="glass-panel p-6">
          <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-6">Invoice Settlement Distribution</h3>
          <div class="flex flex-col md:flex-row items-center justify-around h-64 gap-6">
            <!-- Donut Visualizer -->
            <div class="relative w-40 h-40 rounded-full border-[14px] border-emerald-500 flex items-center justify-center" 
                 [style.border-top-color]="unpaidPercentage() > 0 ? '#f59e0b' : '#10b981'">
              <div class="absolute w-28 h-28 bg-white dark:bg-slate-900 rounded-full flex flex-col items-center justify-center shadow-inner">
                <span class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Total Billed</span>
                <span class="text-lg font-black text-slate-900 dark:text-white">{{ totalInvoiced() | currency:'USD':'symbol':'1.0-0' }}</span>
              </div>
            </div>
            ...
            <div class="space-y-3">
              <div class="flex items-center gap-2">
                ...
                <div>
                  <p class="text-xs font-bold text-slate-900 dark:text-white">Paid Invoices ({{ paidPercentage() }}%)</p>
                  <p class="text-[10px] text-slate-400 font-semibold">{{ paidTotal() | currency }}</p>
                </div>
              </div>
              <div class="flex items-center gap-2">
                <div class="w-3 h-3 bg-amber-500 rounded-full"></div>
                <div>
                  <p class="text-xs font-bold text-slate-900 dark:text-white">Outstanding Invoices ({{ unpaidPercentage() }}%)</p>
                  <p class="text-[10px] text-slate-400 font-semibold">{{ unpaidTotal() | currency }}</p>
                </div>
              </div>
            </div>
          </div>
