...
@Component({
  ...
  template: `
    <div
      class="news-ticker-container border-b border-slate-200/50 dark:border-slate-800/50 bg-white/50 dark:bg-slate-950/50 backdrop-blur-md"
    >
      <div class="flex items-center h-12 container mx-auto px-4 overflow-hidden relative">
        <!-- Label -->
        <div
          class="absolute left-0 z-10 h-full flex items-center px-4 bg-mit-red text-white font-black text-xs uppercase tracking-widest shadow-[10px_0_20px_-5px_rgba(163,31,52,0.3)]"
        >
          Update Notice
        </div>
          ...
          <div class="ticker-content flex gap-12 animate-scroll group-hover:pause">
            @for (news of newsItems(); track $index) {
              <div class="flex items-center gap-3">
                ...
                <span
                  class="text-sm font-bold text-slate-700 dark:text-slate-300 hover:text-mit-red transition-colors cursor-default"
                >
                  {{ news }}
                </span>
              </div>
