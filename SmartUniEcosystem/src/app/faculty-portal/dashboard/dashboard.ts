  ...
  template: `
    <div class="space-y-8 animate-fade-in-up">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight leading-none">Welcome back, {{ user()?.name }}</h1>
          <p class="text-slate-500 mt-2">Manage your academic schedule and assigned institutional tasks.</p>
        </div>
        <button (click)="createNewAssignment()" class="px-5 py-2.5 bg-indigo-600 text-white rounded-xl shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/50 hover:-translate-y-0.5 transition-all font-bold flex items-center gap-2">
        ...
        <div class="lg:col-span-2 space-y-4">
          ...
          <div class="glass-panel overflow-hidden">
            <div class="divide-y divide-slate-200 dark:divide-slate-800">
              <div *ngFor="let log of attendanceLogs()" class="p-4 flex items-center gap-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                <div class="w-16 h-16 rounded-xl bg-indigo-50 dark:bg-indigo-900/30 flex flex-col items-center justify-center shrink-0 border border-indigo-100 dark:border-indigo-800">
                  <span class="text-xs font-bold text-indigo-600 dark:text-indigo-400">{{ log.checkIn }}</span>
                  <span class="text-[10px] font-medium text-slate-500 uppercase">In</span>
                </div>
                <div class="flex-1">
                  <h4 class="font-bold text-slate-900 dark:text-white text-lg">Work Log: {{ log.date }}</h4>
                  <p class="text-sm text-slate-500">Status: {{ log.status }} &bull; Check-out: {{ log.checkOut }}</p>
                </div>
                <span class="px-3 py-1 bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 rounded-full text-xs font-bold">{{ log.status }}</span>
              </div>
              <div *ngIf="attendanceLogs().length === 0" class="p-8 text-center text-slate-500">No attendance records found.</div>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-6">
          <div class="space-y-4">
            ...
            <div class="glass-panel p-5 space-y-4">
              <div *ngFor="let task of tasks()" class="flex gap-3 pb-3 border-b border-slate-100 dark:border-slate-800 last:border-0 last:pb-0">
                <div [ngClass]="{
                  'bg-amber-500': task.priority === 'High',
                  'bg-blue-500': task.priority === 'Medium',
                  'bg-slate-400': task.priority === 'Low'
                }" class="w-2 h-2 rounded-full mt-2 shrink-0"></div>
                <div>
                  ...
                  <div class="flex items-center gap-2 mt-2">
                    <span class="text-[10px] font-bold px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 rounded text-slate-600 dark:text-slate-400">Due: {{ task.dueDate }}</span>
                    <span class="text-[10px] font-bold text-indigo-500 uppercase tracking-tighter">{{ task.status }}</span>
                  </div>
                </div>
              </div>
              <div *ngIf="tasks().length === 0" class="text-center py-4 text-slate-500 text-sm italic">No pending tasks.</div>
            </div>
          </div>

          <div class="space-y-4">
            <h3 class="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-purple-500"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
              My Leave Status
            </h3>
            <div class="glass-panel p-5">
              <div *ngFor="let leave of leaveRequests()" class="flex items-center justify-between mb-3 last:mb-0">
                <div>
                ...
                }" class="px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider">{{ leave.status }}</span>
              </div>
              <div *ngIf="leaveRequests().length === 0" class="text-center text-slate-500 text-xs italic">No leave history.</div>
            </div>
          </div>
        </div>
      </div>
...
export class FacultyDashboardComponent implements OnInit {
  ...
  createNewAssignment() {
    ...
    if (title) {
      alert(`Assignment "${title}" has been drafted. You can finalize it in the Course Management section.`);
    }
  }
}
