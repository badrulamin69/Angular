    ...
    <div class="space-y-6 animate-fade-in-up">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-2">
            <button *ngFor="let assignment of assignments()" (click)="selectAssignment(assignment)" [ngClass]="{'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20': selectedAssignmentId() === assignment.id, 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900': selectedAssignmentId() !== assignment.id}" class="w-full p-3 border rounded-xl text-left transition-colors">
              <p class="font-bold text-sm" [class.text-indigo-700]="selectedAssignmentId() === assignment.id" [class.dark:text-indigo-400]="selectedAssignmentId() === assignment.id" [class.text-slate-900]="selectedAssignmentId() !== assignment.id" [class.dark:text-white]="selectedAssignmentId() !== assignment.id">{{ assignment.title }}</p>
              <p class="text-xs mt-1" [class.text-indigo-500]="selectedAssignmentId() === assignment.id" [class.text-slate-500]="selectedAssignmentId() !== assignment.id">Due: {{ assignment.dueDate }} &bull; {{ assignment.totalPoints }} Points</p>
            </button>
            <div *ngIf="assignments().length === 0" class="p-4 text-center text-slate-500 text-xs border border-dashed rounded-xl">No assignments found for this course.</div>
          </div>
        </div>
        ...
        <div class="lg:col-span-3 space-y-4">
          <div *ngIf="selectedAssignment(); else noAssignment" class="glass-panel overflow-hidden">
            <div class="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
              <h3 class="font-bold text-slate-900 dark:text-white">Submissions ({{ selectedAssignment()?.title }})</h3>
            ...
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="border-b border-slate-200 dark:border-slate-700">
                  ...
                  <th class="px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Grade (/{{ selectedAssignment()?.totalPoints }})</th>
                  <th class="px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Action</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                <tr *ngFor="let sub of submissions()" class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                  <td class="px-6 py-4 font-mono text-sm text-slate-600 dark:text-slate-400">{{ sub.studentId }}</td>
                  <td class="px-6 py-4 font-bold text-slate-900 dark:text-white flex items-center gap-3">
                    <div class="w-8 h-8 rounded-full bg-slate-200 overflow-hidden shrink-0">
                      <img [src]="'https://api.dicebear.com/7.x/avataaars/svg?seed=' + sub.studentName" alt="Avatar" class="w-full h-full">
                    </div>
                    {{ sub.studentName }}
                  </td>
                  <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">
                    <span class="text-emerald-600 dark:text-emerald-400 font-medium">{{ sub.submissionDate }}</span>
                  </td>
                  <td class="px-6 py-4">
                    <a [href]="sub.fileUrl" target="_blank" class="text-indigo-600 dark:text-indigo-400 hover:underline flex items-center gap-1 text-sm font-medium">
                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>
                      {{ sub.fileUrl }}
                    </a>
                  </td>
                  <td class="px-6 py-4">
                    <input type="number" [(ngModel)]="sub.grade" class="w-20 px-2 py-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm text-slate-900 dark:text-white" placeholder="0-100">
                  </td>
                  <td class="px-6 py-4 text-right">
                    <button (click)="saveGrade(sub)" class="px-3 py-1 bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400 rounded-md text-xs font-bold hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors">Save</button>
                  </td>
                </tr>
                <tr *ngIf="submissions().length === 0">
                  <td colspan="6" class="px-6 py-12 text-center text-slate-500 italic">No submissions found for this assignment.</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <ng-template #noAssignment>
            <div class="flex flex-col items-center justify-center py-32 text-center bg-white dark:bg-slate-900 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800">
              ...
              <h2 class="text-xl font-bold text-slate-900 dark:text-white">Select an assignment to grade</h2>
              <p class="text-slate-500 mt-1 max-w-xs">Pick a course and then an assignment from the sidebar to start reviewing student work.</p>
            </div>
          </ng-template>
        </div>
      </div>
