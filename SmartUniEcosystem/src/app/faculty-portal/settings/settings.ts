...
import {
  FacultySettingsService,
  FacultySettings,
  Session,
} from '../../core/services/faculty-settings.service';
...
@Component({
  ...
  template: `
    <div class="space-y-6 animate-fade-in-up max-w-5xl mx-auto pb-16">
      ...
      >
        <div>
          ...
          <p class="text-slate-500 mt-1">
            Manage your professional profile, account security, notification alerts, and theme
            preferences.
          </p>
        </div>
      </div>
      ...
      <div
        *ngIf="error()"
        class="p-6 bg-rose-50 border border-rose-200 dark:bg-rose-950/20 dark:border-rose-900/30 rounded-2xl flex items-center gap-4 text-rose-700 dark:text-rose-400"
      >
        <svg
        ...
        <div>
          ...
          <button (click)="loadData()" class="mt-2 text-xs font-bold underline hover:text-rose-800">
            Retry loading
          </button>
        </div>
      </div>
      ...
      <div
        *ngIf="!loading() && !error()"
        class="glass-panel overflow-hidden flex flex-col md:flex-row"
      >
        <!-- Navigation Menu -->
        <div
          class="w-full md:w-64 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 p-4 space-y-1 shrink-0"
        >
          <button
        ...
        <div class="flex-1 p-6 md:p-8">
          ...
          <div *ngIf="activeTab() === 'profile'" class="space-y-6">
            <div>
              ...
              <p class="text-slate-500 text-sm">
                Update your official faculty profile information.
              </p>
            </div>

            <!-- Profile Photo Section -->
            <div
              class="flex items-center gap-6 pb-6 border-b border-slate-200 dark:border-slate-800"
            >
              <div
                class="relative w-24 h-24 rounded-full border-4 border-white dark:border-slate-800 shadow-xl overflow-hidden shrink-0 bg-slate-100 dark:bg-slate-850 flex items-center justify-center"
              >
                <img
                  *ngIf="avatarSrc"
                  [src]="avatarSrc"
                ...
                />
                <span *ngIf="!avatarSrc" class="text-2xl font-bold text-slate-400 uppercase">{{
                  profileForm.get('name')?.value?.charAt(0) || 'F'
                }}</span>
              </div>
              <div class="space-y-2">
                ...
                <div class="flex gap-2">
                  ...
                  <button
                    ...
                    class="px-4 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg text-xs font-bold transition-colors"
                  >
                    Upload Photo
                  </button>
                  <button
                    *ngIf="avatarSrc"
                    (click)="removeAvatar()"
                    class="px-4 py-1.5 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/20 rounded-lg text-xs font-bold transition-colors"
                  >
                    Remove
                  </button>
                </div>
            ...
            <form [formGroup]="profileForm" (ngSubmit)="saveProfile()" class="space-y-6">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    class="w-full px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 text-sm font-bold text-slate-900 dark:text-white"
                  />
                  <p
                ...
                <div>
                  ...
                  <input
                    type="email"
                    [value]="user()?.email"
                    class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-sm font-semibold text-slate-400 cursor-not-allowed"
                    disabled
                  />
                  <span class="text-[10px] text-slate-400 mt-1 block"
                    >Institutional email cannot be changed.</span
                  >
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    class="w-full px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 text-sm font-semibold text-slate-900 dark:text-white"
                  />
                </div>
                ...
                <div>
                  <label
                    class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2"
                    >Designation <span class="text-rose-500">*</span></label
                  >
                  <input
                    type="text"
              ...
              >
                <button
                  ...
                  class="px-5 py-2 text-sm font-bold border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-350 hover:bg-slate-50 dark:hover:bg-slate-850 rounded-xl transition-all"
                >
                  Discard Changes
                </button>
                <button
                  type="submit"
                  [disabled]="!canSave() || savingProfileState()"
                  class="px-5 py-2 bg-indigo-600 text-white font-bold rounded-xl text-sm shadow-md hover:bg-indigo-700 disabled:opacity-50 transition-all flex items-center gap-2"
                >
                  <svg
                    *ngIf="savingProfileState()"
          ...
          <div *ngIf="activeTab() === 'security'" class="space-y-6">
            ...
            <form [formGroup]="passwordForm" (ngSubmit)="savePassword()" class="space-y-5 max-w-md">
              ...
              <div>
                ...
                <div class="relative">
                  <input
                    ...
                    class="w-full pl-4 pr-12 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 text-sm"
                    placeholder="••••••••"
                  />
                  <button
                    type="button"
                    (click)="showCurrentPassword.set(!showCurrentPassword())"
                    class="absolute right-4 top-3.5 text-slate-400 hover:text-slate-655 focus:outline-none"
                  >
                    <svg
              ...
              <div>
                ...
                <div class="mt-2 text-xs space-y-1">
                  <div
                    ...
                    [class.text-emerald-500]="newPassValidLength()"
                    [class.text-slate-400]="!newPassValidLength()"
                  >
                    <span
                      class="w-1.5 h-1.5 rounded-full"
                      [class.bg-emerald-500]="newPassValidLength()"
                      [class.bg-slate-400]="!newPassValidLength()"
                    ></span>
              ...
              <div
                class="pt-4 flex justify-end gap-3 border-t border-slate-200 dark:border-slate-800"
              >
                <button
                  type="button"
                ...
                <button
                  type="submit"
                  [disabled]="passwordForm.invalid || savingPasswordState()"
                  class="px-5 py-2 bg-indigo-600 text-white font-bold rounded-xl text-sm shadow-md hover:bg-indigo-700 disabled:opacity-50 transition-all flex items-center gap-2"
                >
                  <svg
                    *ngIf="savingPasswordState()"
                    class="animate-spin h-4 w-4"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
            ...
            <form [formGroup]="preferencesForm" (ngSubmit)="savePreferences()" class="space-y-8">
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <p class="text-slate-500 text-xs mt-0.5">
                    Customize your desktop portal theme and layout behavior.
                  </p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <!-- Light Theme Card -->
                  <label
                    class="relative border-2 rounded-2xl p-4 flex flex-col items-center gap-3 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/40 select-none"
                  ...
                  >
                    ...
                    <span
                      class="w-8 h-8 rounded-full bg-white border border-slate-300 flex items-center justify-center text-slate-700 shadow-md"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                    ...
                    <span class="font-bold text-sm text-slate-900 dark:text-white mt-1"
                      >Light Mode</span
                    >
                  </label>
                  ...
                  >
                    ...
                    <span
                      class="w-8 h-8 rounded-full bg-slate-900 border border-slate-750 flex items-center justify-center text-indigo-400 shadow-md"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                  ...
                  >
                    ...
                    <span
                      class="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-600 dark:text-slate-300 shadow-md"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                ...
                <!-- Sidebar Toggles -->
                <div
                  class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800"
                >
                  <div>
                    <span class="font-bold text-sm text-slate-800 dark:text-slate-250"
                      >Collapse Sidebar by Default</span
                    >
                    <p class="text-xs text-slate-500 mt-0.5">
                      Keep the sidebar compressed automatically to expand your workspace area.
                    </p>
                  </div>
                  <button
                    type="button"
                    (click)="togglePrefControl('sidebarCollapsed')"
                    class="relative shrink-0 ml-4 w-11 h-6 rounded-full transition-colors duration-200 focus:outline-none"
                    [class.bg-indigo-600]="preferencesForm.get('sidebarCollapsed')?.value"
                    [class.bg-slate-250]="!preferencesForm.get('sidebarCollapsed')?.value"
                    [class.dark:bg-slate-700]="!preferencesForm.get('sidebarCollapsed')?.value"
                  >
                    <span
                      class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200"
                      [class.translate-x-5]="preferencesForm.get('sidebarCollapsed')?.value"
                      [class.translate-x-0]="!preferencesForm.get('sidebarCollapsed')?.value"
                    ></span>
                  </button>
                </div>
              </div>
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <p class="text-slate-500 text-xs mt-0.5">
                    Control which updates and alerts you wish to receive from the ecosystem.
                  </p>
                </div>
                ...
                <div class="space-y-3">
                  ...
                  >
                    <div>
                      ...
                      >
                      <p class="text-xs text-slate-500 mt-0.5">
                        Receive daily summaries, announcements, and university bulletins.
                      </p>
                    </div>
                    <button
                      type="button"
                      (click)="togglePrefControl('emailNotifications')"
                      class="relative shrink-0 ml-4 w-11 h-6 rounded-full transition-colors duration-200 focus:outline-none"
                      [class.bg-indigo-600]="preferencesForm.get('emailNotifications')?.value"
                      [class.bg-slate-250]="!preferencesForm.get('emailNotifications')?.value"
                      [class.dark:bg-slate-700]="!preferencesForm.get('emailNotifications')?.value"
                    >
                      <span
                        class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200"
                        [class.translate-x-5]="preferencesForm.get('emailNotifications')?.value"
                        [class.translate-x-0]="!preferencesForm.get('emailNotifications')?.value"
                      ></span>
                    </button>
                  </div>
                  ...
                  >
                    <div>
                      <span class="font-bold text-sm text-slate-800 dark:text-slate-250"
                        >Assignment Submissions Alert</span
                      >
                      <p class="text-xs text-slate-500 mt-0.5">
                        Get notified instantly when students submit course assignments.
                      </p>
                    </div>
                    <button
                      type="button"
                      (click)="togglePrefControl('assignmentNotifications')"
                      class="relative shrink-0 ml-4 w-11 h-6 rounded-full transition-colors duration-200 focus:outline-none"
                      [class.bg-indigo-600]="preferencesForm.get('assignmentNotifications')?.value"
                      [class.bg-slate-250]="!preferencesForm.get('assignmentNotifications')?.value"
                      [class.dark:bg-slate-700]="
                        !preferencesForm.get('assignmentNotifications')?.value
                      "
                    >
                      <span
                        class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200"
                  ...
                  >
                    <div>
                      <span class="font-bold text-sm text-slate-800 dark:text-slate-250"
                        >Exam Schedules & Reminders</span
                      >
                      <p class="text-xs text-slate-500 mt-0.5">
                        Receive alerts when exam papers are published or grades are finalized.
                      </p>
                    </div>
                    <button
                      type="button"
                      (click)="togglePrefControl('examNotifications')"
                      class="relative shrink-0 ml-4 w-11 h-6 rounded-full transition-colors duration-200 focus:outline-none"
                      [class.bg-indigo-600]="preferencesForm.get('examNotifications')?.value"
                      [class.bg-slate-250]="!preferencesForm.get('examNotifications')?.value"
                      [class.dark:bg-slate-700]="!preferencesForm.get('examNotifications')?.value"
                    >
                      <span
                        class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200"
                        [class.translate-x-5]="preferencesForm.get('examNotifications')?.value"
                        [class.translate-x-0]="!preferencesForm.get('examNotifications')?.value"
                      ></span>
                    </button>
                  </div>
                  ...
                  >
                    <div>
                      <span class="font-bold text-sm text-slate-800 dark:text-slate-250"
                        >Student Message Alerts</span
                      >
                      <p class="text-xs text-slate-500 mt-0.5">
                        Receive direct email notifications when a student sends you a query.
                      </p>
                    </div>
                    <button
                      type="button"
                      (click)="togglePrefControl('messageNotifications')"
                      class="relative shrink-0 ml-4 w-11 h-6 rounded-full transition-colors duration-200 focus:outline-none"
                      [class.bg-indigo-600]="preferencesForm.get('messageNotifications')?.value"
                      [class.bg-slate-250]="!preferencesForm.get('messageNotifications')?.value"
                      [class.dark:bg-slate-700]="
                        !preferencesForm.get('messageNotifications')?.value
                      "
                    >
                      <span
                        class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200"
                        [class.translate-x-5]="preferencesForm.get('messageNotifications')?.value"
                        [class.translate-x-0]="!preferencesForm.get('messageNotifications')?.value"
                      ></span>
                    </button>
                  </div>
                </div>
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <p class="text-slate-500 text-xs mt-0.5">
                    Control the visibility of your profile information to students and public
                    search.
                  </p>
                </div>
                ...
                <div class="space-y-3">
                  ...
                  >
                    <div>
                      <span class="font-bold text-sm text-slate-800 dark:text-slate-250"
                        >Public Profile Visibility</span
                      >
                      <p class="text-xs text-slate-500 mt-0.5">
                        Show your profile in the public student advisory directories.
                      </p>
                    </div>
                    <button
                      type="button"
                      (click)="togglePrefControl('profileVisibility')"
                      class="relative shrink-0 ml-4 w-11 h-6 rounded-full transition-colors duration-200 focus:outline-none"
                      [class.bg-indigo-600]="preferencesForm.get('profileVisibility')?.value"
                      [class.bg-slate-250]="!preferencesForm.get('profileVisibility')?.value"
                      [class.dark:bg-slate-700]="!preferencesForm.get('profileVisibility')?.value"
                    >
                      <span
                        class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200"
                        [class.translate-x-5]="preferencesForm.get('profileVisibility')?.value"
                        [class.translate-x-0]="!preferencesForm.get('profileVisibility')?.value"
                      ></span>
                    </button>
                  </div>
                  ...
                  >
                    <div>
                      <span class="font-bold text-sm text-slate-800 dark:text-slate-250"
                        >Display Contact Information</span
                      >
                      <p class="text-xs text-slate-500 mt-0.5">
                        Expose your phone number to enrolled students for academic advising.
                      </p>
                    </div>
                    <button
                      type="button"
                      (click)="togglePrefControl('phoneVisibility')"
                      class="relative shrink-0 ml-4 w-11 h-6 rounded-full transition-colors duration-200 focus:outline-none"
                      [class.bg-indigo-600]="preferencesForm.get('phoneVisibility')?.value"
                      [class.bg-slate-250]="!preferencesForm.get('phoneVisibility')?.value"
                      [class.dark:bg-slate-700]="!preferencesForm.get('phoneVisibility')?.value"
                    >
                      <span
                        class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200"
                        [class.translate-x-5]="preferencesForm.get('phoneVisibility')?.value"
                        [class.translate-x-0]="!preferencesForm.get('phoneVisibility')?.value"
                      ></span>
                    </button>
                  </div>
                </div>
              ...
              >
                ...
                <button
                  type="submit"
                  [disabled]="preferencesForm.invalid || savingPreferencesState()"
                  class="px-5 py-2 bg-indigo-600 text-white font-bold rounded-xl text-sm shadow-md hover:bg-indigo-700 disabled:opacity-50 transition-all flex items-center gap-2"
                >
                  <svg
                    *ngIf="savingPreferencesState()"
                    class="animate-spin h-4 w-4"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
          ...
          <div *ngIf="activeTab() === 'sessions'" class="space-y-6 animate-fade-in">
            <div
              class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-slate-200 dark:border-slate-800 pb-5"
            >
              <div>
                <h3 class="text-xl font-bold text-slate-900 dark:text-white">Active Sessions</h3>
                <p class="text-slate-500 text-sm">
                  Review devices that are currently authenticated to access your account.
                </p>
              </div>
              <button
                *ngIf="otherSessions().length > 0"
                (click)="confirmLogoutOtherDevices()"
                class="px-4 py-2 text-xs font-bold bg-rose-50 border border-rose-200 hover:bg-rose-100 text-rose-600 dark:bg-rose-950/20 dark:border-rose-900/30 dark:text-rose-450 dark:hover:bg-rose-900/20 rounded-xl transition-colors shrink-0"
              >
            ...
            <div class="space-y-4">
              <div
                *ngFor="let sess of sessions()"
                class="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-start justify-between gap-4"
              >
                <div class="flex gap-4">
                  <div
                    class="w-10 h-10 rounded-xl bg-slate-200/60 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-350 shrink-0"
                  >
                    <svg
                      *ngIf="
                  ...
                  <div>
                    <div class="flex items-center gap-2 flex-wrap">
                      ...
                      <span
                        *ngIf="sess.current"
                        class="px-2 py-0.5 text-[10px] font-black uppercase bg-emerald-100 text-emerald-800 dark:bg-emerald-950/45 dark:text-emerald-400 rounded-md"
                        >Current Session</span
                      >
                    </div>
                    <p class="text-xs text-slate-500 mt-1">
                      IP: {{ sess.ipAddress }} &bull; Location: {{ sess.location }}
                    </p>
                ...
                <button
                  *ngIf="!sess.current"
                  (click)="confirmLogoutDevice(sess)"
                  class="p-2 text-slate-400 hover:text-rose-500 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors shrink-0"
            ...
            <!-- Last Login details -->
            <div
              class="p-4 bg-slate-100/50 dark:bg-slate-850/50 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 text-slate-500 text-xs flex items-center gap-3"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
              ...
              <span
                ><strong>Last Login:</strong> {{ lastLoginTime() }} &bull; Check active logs
                regularly to monitor security.</span
              >
            </div>
          </div>
        </div>
    ...
    >
      <div
        class="glass border border-slate-200 dark:border-slate-800 rounded-3xl p-6 max-w-md w-full shadow-2xl animate-fade-in-up space-y-4"
      >
        ...
        <div class="flex justify-end gap-3 pt-2">
          <button
            ...
            class="px-4 py-2 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-350 hover:bg-slate-50 dark:hover:bg-slate-850 rounded-xl text-sm font-bold transition-all"
          >
            Cancel
          </button>
          <button
            (click)="closeConfirmModal(true)"
            class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-sm transition-all"
          >
            Confirm
          </button>
        </div>
...
export class FacultySettingsComponent implements OnInit {
  ...
  canSave = computed(() => {
    try {
      ...
      const profileChanged = this.originalProfileValue
        ? JSON.stringify(this.profileForm.value) !== JSON.stringify(this.originalProfileValue)
        : true;
  ...
  hasUnsavedChanges(): boolean {
    return (
      (this.profileForm.dirty &&
        JSON.stringify(this.profileForm.value) !== JSON.stringify(this.originalProfileValue)) ||
      (this.preferencesForm.dirty &&
        JSON.stringify(this.preferencesForm.value) !==
          JSON.stringify(this.originalPreferencesValue)) ||
