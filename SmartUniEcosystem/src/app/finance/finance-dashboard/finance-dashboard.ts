  ...
  template: `
    <div class="space-y-6">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <button (click)="downloadReport()" class="px-4 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all font-bold flex items-center gap-2">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in delay-100">
        ...
        <div class="glass-card p-6 border-l-4 border-l-blue-500 hover:-translate-y-1 transition-transform cursor-pointer">
          ...
          <h2 class="text-3xl font-black text-slate-900 dark:text-white mt-4">{{ scholarshipsTotal() | currency:'USD':'symbol':'1.1-1' }}</h2>
      ...
      <div class="glass-panel overflow-hidden animate-fade-in delay-200 mt-8">
        <div class="p-6 border-b border-slate-200 dark:border-slate-700/50 flex justify-between items-center">
          ...
          <button (click)="openInvoicesModal()" class="text-mit-red font-bold text-sm hover:underline">View All</button>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-700/50">
              <tr *ngFor="let i of recentInvoices()" (click)="printInvoice(i)" class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer">
                <td class="px-6 py-4 font-bold text-slate-700 dark:text-slate-300">{{ i.id }}</td>
                <td class="px-6 py-4 font-bold text-slate-900 dark:text-white">{{ i.studentName }}</td>
                ...
                <td class="px-6 py-4">
                  <span [ngClass]="{
                    ...
                    'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400': i.status === 'Unpaid' || i.status === 'Pending',
                ...
                <td class="px-6 py-4 text-right">
                  <button (click)="printInvoice(i); $event.stopPropagation()" class="p-1 text-slate-400 hover:text-mit-red transition-colors" title="Download Invoice PDF">
    ...
    <div *ngIf="isInvoicesModalOpen()" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm" (click)="closeInvoicesModal()"></div>
      <div class="glass-panel w-full max-w-3xl p-6 relative z-10 animate-fade-in-up flex flex-col max-h-[80vh] overflow-hidden">
        <div class="flex items-center justify-between mb-6 border-b border-slate-200 dark:border-slate-800 pb-4">
          <div>
            <h3 class="text-xl font-bold text-slate-900 dark:text-white">Tuition & Invoices Ledger</h3>
            <p class="text-xs text-slate-500 mt-0.5">Explore, search, and download student tuition invoices.</p>
          </div>
          <button (click)="closeInvoicesModal()" class="text-slate-400 hover:text-slate-700 dark:hover:text-white transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" x2="6" y1="6" y2="18"/><line x1="6" x2="18" y1="6" y2="18"/></svg>
          </button>
        </div>

        <div class="flex gap-4 mb-4">
          <input type="text" [ngModel]="searchQuery()" (ngModelChange)="searchQuery.set($event)" placeholder="Search by student name or ID..." class="flex-1 px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm outline-none">
          <select [ngModel]="statusFilter()" (ngModelChange)="statusFilter.set($event)" class="px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm outline-none font-bold">
        ...
        <div class="flex-1 overflow-y-auto pr-2 scrollbar-thin">
          <table class="w-full text-left text-xs border-collapse">
            <thead>
              <tr class="bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 font-bold text-slate-500">
            ...
            <tbody>
              <tr *ngFor="let i of filteredInvoices()" (click)="printInvoice(i)" class="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/30 cursor-pointer">
                <td class="px-4 py-3 font-bold text-slate-700 dark:text-slate-300">{{ i.id }}</td>
                <td class="px-4 py-3 font-bold text-slate-900 dark:text-white">{{ i.studentName }}</td>
                <td class="px-4 py-3 text-slate-500">{{ i.date }}</td>
                <td class="px-4 py-3 font-medium">{{ i.credits }} Cr</td>
                <td class="px-4 py-3 font-bold text-slate-900 dark:text-white">{{ i.amount | currency }}</td>
                <td class="px-4 py-3">
                  <span [ngClass]="{
                    'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400': i.status === 'Paid',
                    'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400': i.status === 'Unpaid' || i.status === 'Pending',
                ...
                <td class="px-4 py-3 text-right">
                  <button (click)="printInvoice(i); $event.stopPropagation()" class="p-1 text-slate-400 hover:text-mit-red transition-colors" title="Download Invoice">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>
                  </button>
                </td>
              </tr>
              <tr *ngIf="filteredInvoices().length === 0">
                <td colspan="7" class="px-4 py-8 text-center text-slate-400">No invoices match your search filters.</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export class FinanceDashboardComponent implements OnInit {
  ...
  filteredInvoices = computed(() => {
    ...
    return this.invoices().filter(i => {
      const matchesSearch = i.studentName.toLowerCase().includes(q) || i.id.toLowerCase().includes(q);
  ...
  ngOnInit() {
    ...
    this.http.get<any[]>('http://localhost:3000/invoices').subscribe(data => {
      ...
      const pending = data.filter(i => i.status === 'Pending' || i.status === 'Unpaid' || i.status === 'Overdue')
