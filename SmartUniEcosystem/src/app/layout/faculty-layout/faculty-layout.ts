    ...
    <div class="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300 flex">
      ...
      <aside [ngClass]="{'w-64': isSidebarOpen(), 'w-20': !isSidebarOpen()}" 
        ...
        <div class="h-20 flex items-center justify-center border-b border-slate-200 dark:border-slate-800 px-4 shrink-0">
          <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-xl shadow-lg shrink-0">
            F
          </div>
          <div *ngIf="isSidebarOpen()" class="ml-3 flex flex-col animate-fade-in whitespace-nowrap overflow-hidden">
            <span class="font-black text-lg text-slate-900 dark:text-white leading-tight">Faculty Portal</span>
            <span class="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">Academic OS</span>
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto py-6 px-3 custom-scrollbar">
          <div class="space-y-8">
            ...
            <div>
              ...
              <ul class="space-y-1">
                <li>
                  <a routerLink="/faculty/dashboard" routerLinkActive="bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400 font-bold" class="flex items-center px-3 py-2.5 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-white transition-all group">
                ...
                <li>
                  <a routerLink="/faculty/courses" routerLinkActive="bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400 font-bold" class="flex items-center px-3 py-2.5 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-white transition-all group">
            ...
            <!-- ACADEMIC MANAGEMENT -->
            <div>
              <p *ngIf="isSidebarOpen()" class="px-3 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3">Academic Management</p>
              <ul class="space-y-1">
                <li>
                  <a routerLink="/faculty/attendance" routerLinkActive="bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400 font-bold" class="flex items-center px-3 py-2.5 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-white transition-all group">
                    ...
                    <span *ngIf="isSidebarOpen()" class="ml-3">Attendance</span>
                  </a>
                </li>
                <li>
                  <a routerLink="/faculty/grading" routerLinkActive="bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400 font-bold" class="flex items-center px-3 py-2.5 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-white transition-all group">
                    ...
                    <span *ngIf="isSidebarOpen()" class="ml-3">Grades & Results</span>
                  </a>
                </li>
              </ul>
            </div>
            ...
            <!-- EXAMINATIONS -->
            <div>
              <p *ngIf="isSidebarOpen()" class="px-3 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3">Examinations</p>
              <ul class="space-y-1">
                <li>
                  <a routerLink="/faculty/exams" routerLinkActive="bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400 font-bold" class="flex items-center px-3 py-2.5 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-white transition-all group">
                    ...
                    <span *ngIf="isSidebarOpen()" class="ml-3">Exam Builder</span>
                  </a>
                </li>
              </ul>
            </div>
            ...
            <!-- ADVISING -->
            <div>
              <p *ngIf="isSidebarOpen()" class="px-3 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3">Advising</p>
              <ul class="space-y-1">
                <li>
                  <a routerLink="/faculty/advising" routerLinkActive="bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400 font-bold" class="flex items-center px-3 py-2.5 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-white transition-all group">
                    ...
                    <span *ngIf="isSidebarOpen()" class="ml-3">Student Advising</span>
                  </a>
                </li>
              </ul>
            </div>
      ...
      <div class="flex-1 flex flex-col min-w-0 transition-all duration-300" [class.ml-0]="!isSidebarOpen()">
        ...
        <header class="h-20 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 sticky top-0 z-30">
          <div class="flex items-center gap-4">
            ...
            <div class="hidden sm:flex items-center bg-slate-100 dark:bg-slate-800 rounded-full px-4 py-2 w-64 border border-transparent focus-within:border-indigo-500 focus-within:bg-white dark:focus-within:bg-slate-900 transition-all">
              ...
              <input type="text" placeholder="Search courses or students..." class="bg-transparent border-none outline-none text-sm ml-2 w-full text-slate-700 dark:text-slate-300">
            </div>
          </div>
            ...
            <div class="relative pl-4 border-l border-slate-200 dark:border-slate-800">
              <div (click)="toggleDropdown($event)" class="flex items-center gap-3 cursor-pointer p-1 pr-4 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors border border-transparent">
                <div class="text-right hidden sm:block">
                  <p class="text-sm font-bold text-slate-900 dark:text-white leading-none">{{ user()?.name || 'Dr. Sarah' }}</p>
              ...
              <div *ngIf="isDropdownOpen()" (click)="$event.stopPropagation()" class="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-900 rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.15)] dark:shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] border border-slate-200/50 dark:border-slate-800/50 py-2 animate-fade-in-up z-50 backdrop-blur-xl">
                <div class="px-4 py-3 border-b border-slate-100 dark:border-slate-800 mb-1">
                  <p class="text-sm font-bold text-slate-900 dark:text-white truncate">{{ user()?.name || 'Dr. Sarah' }}</p>
...
export class FacultyLayoutComponent implements OnInit {
  ...
  onResize() {
    if (typeof window !== 'undefined') {
      ...
      if (this.isMobile()) {
        this.isSidebarOpen.set(true); // Keep internal state true for mobile full width
      }
    }
  }
