      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-4">
          <div
            *ngFor="let assign of assignments()"
            class="glass-panel p-6 hover:shadow-lg transition-all relative overflow-hidden group"
          >
            <div
              class="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-slate-100 dark:from-slate-800 to-transparent rounded-bl-full -z-10 group-hover:from-sky-500/10 transition-colors"
            ></div>
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div class="space-y-1">
                <div class="flex items-center gap-2">
                  <span
                    class="px-2 py-0.5 bg-sky-50 dark:bg-sky-900/30 text-sky-600 dark:text-sky-400 text-[10px] font-black uppercase tracking-widest rounded"
                    >{{ assign.courseId }}</span
                  >
              ...
              <div class="flex items-center gap-3">
                <div *ngIf="getSubmission(assign.id); else noSubmission" class="text-right">
                  <span
                    class="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 text-xs font-bold rounded-full"
                  >
                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Submitted
                  </span>
                  <p class="text-[10px] font-bold text-slate-400 mt-1">
                    Grade: {{ getSubmission(assign.id).grade || 'Pending' }}
                  </p>
                </div>
                <ng-template #noSubmission>
                  <button
                    (click)="submitAssignment(assign.id)"
                    class="px-5 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl font-bold text-sm shadow-lg hover:-translate-y-0.5 transition-all"
                  >
                    Submit Work
                  </button>
                </ng-template>
        ...
        <div class="space-y-4">
          <div class="glass-card p-6 border-b-4 border-b-sky-500">
            ...
            <div
              class="w-full bg-slate-100 dark:bg-slate-800 h-2 rounded-full mt-4 overflow-hidden"
            >
              <div
          ...
          <div class="glass-panel p-5">
            <h4
              class="font-bold text-slate-900 dark:text-white mb-3 text-sm uppercase tracking-wider"
            >
              Upcoming Deadlines
            </h4>
            <div class="space-y-3">
              <div
                *ngFor="let d of upcomingDeadlines()"
                class="flex items-center gap-3 p-2 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-lg transition-colors cursor-pointer"
              >
                <div
                  class="w-10 h-10 rounded-xl bg-rose-50 dark:bg-rose-900/20 text-rose-500 flex flex-col items-center justify-center shrink-0"
                >
                ...
                <div>
                  <p class="text-xs font-bold text-slate-900 dark:text-white line-clamp-1">
                    {{ d.title }}
                  </p>
