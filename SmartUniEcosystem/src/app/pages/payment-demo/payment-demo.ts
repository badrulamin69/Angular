  ...
  template: `
    <div class="min-h-screen pt-24 pb-12 bg-slate-50 dark:bg-slate-950 px-4">
      <div class="max-w-xl mx-auto">
        <div
          class="glass-card rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-2xl"
        >
          <!-- ---- STEP: SELECT ---- -->
          <ng-container *ngIf="step() === 'select'">
            ...
            <div class="p-8 space-y-4">
              <!-- Amount -->
              <div
                class="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-slate-800"
              >
              ...
              <p class="text-sm font-semibold text-slate-500 uppercase tracking-wider pt-2">
              ...
              >
                <div class="flex gap-1.5">
                  ...
                  <div
                    class="w-10 h-7 rounded bg-orange-500 flex items-center justify-center text-white text-[8px] font-black"
                  >
                    MC
                  </div>
                </div>
          ...
          <ng-container *ngIf="step() === 'form'">
            <div [ngClass]="getHeaderClass()" class="p-6 text-white">
              <button
                (click)="step.set('select')"
                class="flex items-center gap-1 text-white/70 hover:text-white text-sm mb-3 transition-colors"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="14"
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div class="flex gap-2">
                      <select
                        [(ngModel)]="expiryMonth"
                        class="flex-1 px-2 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500"
                      >
            ...
            <div class="p-16 flex flex-col items-center justify-center gap-6">
              ...
              <div class="text-center">
                ...
                <p class="text-sm text-slate-500 mt-1">
                  Connecting to {{ getMethodLabel() }} gateway
                </p>
              </div>
            </div>
          </ng-container>
          ...
          <ng-container *ngIf="step() === 'success'">
            <div class="p-8 space-y-5">
              ...
              <a
                routerLink="/student/finance"
                class="block w-full py-3.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-bold text-center hover:scale-[1.01] transition-all shadow-lg"
              >
  ...
  processPayment() {
    ...
    if (this.selectedMethod() === 'card') {
      if (this.cardNumber.replace(/\s/g, '').length < 16) {
        this.formError.set('Enter a valid 16-digit card number.');
        return;
      }
      if (!this.cardName.trim()) {
        this.formError.set('Enter the cardholder name.');
        return;
      }
      if (!this.expiryMonth || !this.expiryYear) {
        this.formError.set('Select card expiry date.');
        return;
      }
      if (this.cvv.length < 3) {
        this.formError.set('Enter a valid CVV.');
        return;
      }
    } else {
      ...
      if (this.mobilePin.length < 4) {
        this.formError.set('Enter your PIN.');
        return;
      }
    }
