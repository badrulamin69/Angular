    ...
    <div class="min-h-screen pt-24 pb-12 bg-slate-50 dark:bg-slate-950 px-4">
      <div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div class="glass-panel p-8 md:p-12 animate-fade-in-up">
          <div class="mb-10">
            <h1 class="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Institutional Registration</h1>
            <p class="text-slate-500 mt-2 font-medium">Complete your academic profile to generate your official student credentials and enrollment receipt.</p>
          </div>
          ...
          <form [formGroup]="regForm" (ngSubmit)="onFormSubmit()" class="space-y-6">
            <div class="space-y-6">
              <div>
                ...
                <input type="text" formControlName="fullName" placeholder="e.g. Badrul Amin" class="w-full px-5 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl outline-none focus:ring-2 focus:ring-mit-red text-slate-900 dark:text-white font-medium transition-all shadow-sm">
              </div>

              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Email Address</label>
                  <input type="email" formControlName="email" placeholder="student@university.edu" class="w-full px-5 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl outline-none focus:ring-2 focus:ring-mit-red text-slate-900 dark:text-white font-medium transition-all shadow-sm">
                </div>
                <div>
                  <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Phone Number</label>
                  <input type="tel" formControlName="phone" placeholder="+880 1709-628913" class="w-full px-5 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl outline-none focus:ring-2 focus:ring-mit-red text-slate-900 dark:text-white font-medium transition-all shadow-sm">
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="date" formControlName="dob" class="w-full px-5 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl outline-none focus:ring-2 focus:ring-mit-red text-slate-900 dark:text-white font-medium transition-all shadow-sm">
                </div>
                <div>
        ...
        <div class="flex flex-col items-center justify-center space-y-8 animate-fade-in delay-200">
          <div class="relative group">
            ...
            <div class="absolute -inset-1 bg-gradient-to-r from-mit-red to-primary-600 rounded-[3rem] blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200"></div>
            
            <div #idCard class="w-[380px] h-[520px] rounded-[2.5rem] bg-white dark:bg-slate-900 shadow-2xl overflow-hidden relative border border-slate-100 dark:border-slate-800 z-10 transition-transform duration-500 group-hover:scale-[1.01]">
              <!-- MIT Red Top Header -->
              <div class="h-36 bg-mit-red relative flex flex-col items-center justify-center pt-4">
                <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center backdrop-blur-md mb-2">
                  <span class="text-white font-black text-xl">S</span>
                </div>
                <p class="text-[10px] font-black uppercase tracking-[0.4em] text-white opacity-80">University Management</p>
              ...
              <div class="mt-28 px-10 text-center">
                <h3 class="text-2xl font-black text-slate-900 dark:text-white leading-tight mb-1">{{ regForm.value.fullName || 'BADRUL AMIN' }}</h3>
                <p class="text-xs font-black text-mit-red uppercase tracking-widest mb-10">{{ regForm.value.department || 'ACADEMIC DEPARTMENT' }}</p>
                
                <div class="space-y-5 text-left border-t border-slate-100 dark:border-slate-800 pt-8">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Student Email</p>
                      <p class="text-[13px] font-bold text-slate-700 dark:text-slate-300 truncate w-48">{{ regForm.value.email || 'student@university.edu' }}</p>
                    </div>
                  </div>
                  ...
                  <div class="flex justify-between items-center">
                    <div class="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p class="text-[13px] font-bold text-slate-700 dark:text-slate-300">{{ regForm.value.phone || '+880 1709-628913' }}</p>
                      </div>
                    </div>
                    <div class="text-right">
                      <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Date of Birth</p>
                      <p class="text-[13px] font-bold text-slate-700 dark:text-slate-300">{{ regForm.value.dob || '2026-05-18' }}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="absolute bottom-8 right-8">
                <div class="w-12 h-12 border-2 border-slate-100 dark:border-slate-800 rounded-xl flex items-center justify-center opacity-40">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect width="5" height="5" x="3" y="3" rx="1"/><rect width="5" height="5" x="16" y="3" rx="1"/><rect width="5" height="5" x="3" y="16" rx="1"/><path d="M21 16V3"/><path d="M3 21h18"/><path d="M10 10v4h4"/></svg>
                </div>
              </div>
            </div>
          </div>
          
          <div class="flex items-center gap-3 px-6 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm max-w-[380px]">
            <div class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
            <p class="text-[10px] text-slate-500 font-black uppercase tracking-widest">Official Registration Active</p>
          </div>
        </div>
      ...
      <div style="position: absolute; left: -9999px; top: -9999px;">
        <div #printReceipt class="bg-white p-16" style="width: 794px; min-height: 1123px; color: #0f172a; font-family: sans-serif;">
          <!-- Header -->
          <div class="flex justify-between items-start border-b-2 border-mit-red pb-8 mb-12">
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <p class="text-sm font-bold text-slate-500 tracking-[0.2em]">OFFICIAL REGISTRATION RECEIPT</p>
              </div>
            </div>
            <div class="text-right">
          ...
          <div class="space-y-12">
            <div>
              <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-6 border-b border-slate-100 pb-2">Student Information</h3>
            ...
            <div>
              <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-6 border-b border-slate-100 pb-2">Institutional Credentials</h3>
              <div class="p-8 bg-slate-50 rounded-3xl border border-slate-100 flex items-center justify-between">
                <div class="space-y-4">
                  <div>
                    <p class="text-[10px] font-black text-slate-400 uppercase">System ID Card Status</p>
                    <p class="text-sm font-black text-green-600 uppercase tracking-wider">Generated & Activated</p>
                  </div>
                  <div>
                    <p class="text-[10px] font-black text-slate-400 uppercase">Enrollment Status</p>
                    <p class="text-sm font-black text-slate-900 uppercase tracking-wider">Active Academic Year 2026-27</p>
                  </div>
                </div>
                <!-- Mini QR -->
                <div class="w-24 h-24 border border-slate-200 rounded-2xl flex items-center justify-center bg-white">
            ...
            <div class="pt-12">
              <div class="border-2 border-slate-900 p-8 rounded-3xl">
                <p class="text-[10px] font-black text-slate-400 uppercase mb-4">Official Declaration</p>
                <p class="text-xs leading-relaxed text-slate-600 italic">
                  "This document serves as primary proof of registration at Smart University. The student mentioned above is hereby granted access to campus facilities, library resources, and digital learning platforms. This document must be presented along with a valid ID card for all official academic purposes."
                </p>
              </div>
            </div>
          </div>
...
export class StudentRegistrationComponent {
  ...
  async onFormSubmit() {
    ...
    try {
      ...
      alert('PROPERLY GENERATED: Your Official Registration Receipt has been downloaded as a high-fidelity PDF.');
