    ...
    <div class="space-y-8 animate-fade-in-up">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-slate-500 mt-1">Welcome back, {{ user()?.name }}! Here is your academic progress.</p>
        </div>
        <button routerLink="/student/registration" class="px-5 py-2.5 bg-gradient-to-r from-mit-red to-primary-600 text-white rounded-xl shadow-lg shadow-mit-red/30 hover:shadow-mit-red/50 hover:-translate-y-0.5 transition-all font-bold flex items-center gap-2">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="glass-card p-6 border-t-4 border-t-mit-red hover:-translate-y-1 transition-transform cursor-pointer group">
          ...
          <h3 class="text-4xl font-black text-slate-900 dark:text-white">{{ cgpa() | number:'1.2-2' }}</h3>
        ...
        <div class="glass-card p-6 border-t-4 border-t-blue-500 hover:-translate-y-1 transition-transform cursor-pointer group">
          ...
          <h3 class="text-4xl font-black text-slate-900 dark:text-white">{{ totalCredits() }}<span class="text-lg text-slate-400">/140</span></h3>
          <div class="w-full bg-slate-200 dark:bg-slate-700 h-1.5 rounded-full mt-3 overflow-hidden">
            <div class="bg-blue-500 h-full rounded-full" [style.width.%]="(totalCredits() / 140) * 100"></div>
          </div>
        </div>
        ...
        <div class="glass-card p-6 border-t-4 border-t-amber-500 hover:-translate-y-1 transition-transform cursor-pointer group">
          <p class="text-sm font-bold text-slate-500 uppercase tracking-wider mb-2 group-hover:text-amber-500 transition-colors">Active Courses</p>
          <h3 class="text-4xl font-black text-slate-900 dark:text-white">{{ enrollments().length }}</h3>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div *ngFor="let enroll of enrollments()" class="glass-panel p-5 relative overflow-hidden group cursor-pointer hover:scale-[1.02] transition-transform">
              ...
              <div class="flex justify-between items-start mb-2">
                <span class="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-300 text-xs font-black tracking-wider rounded-md border border-slate-200 dark:border-slate-700">{{ enroll.courseId }}</span>
                <span class="text-xs font-bold text-slate-500">{{ enroll.semester }}</span>
              </div>
              <h4 class="font-bold text-slate-900 dark:text-white text-lg leading-tight mb-4">{{ enroll.courseTitle || 'Loading Course...' }}</h4>
              
              <div class="space-y-3">
                <div>
                  <div class="flex justify-between text-xs mb-1 font-medium text-slate-500">
                    <span>Attendance</span>
                    <span class="text-slate-700 dark:text-slate-300 font-bold">{{ enroll.attendance }}%</span>
                  </div>
                  <div class="w-full bg-slate-100 dark:bg-slate-800 h-1 rounded-full overflow-hidden">
                    <div class="bg-blue-500 h-full rounded-full" [style.width.%]="enroll.attendance"></div>
                  </div>
                </div>
                <div>
                  <div class="flex justify-between text-xs mb-1 font-medium text-slate-500">
                    <span>Course Progress</span>
                    <span class="text-slate-700 dark:text-slate-300 font-bold">{{ enroll.progress }}%</span>
                  </div>
                  <div class="w-full bg-slate-100 dark:bg-slate-800 h-1 rounded-full overflow-hidden">
                    <div class="bg-emerald-500 h-full rounded-full" [style.width.%]="enroll.progress"></div>
                  </div>
                </div>
              </div>
            </div>
            <div *ngIf="enrollments().length === 0" class="col-span-2 p-12 text-center text-slate-500 glass-panel">
