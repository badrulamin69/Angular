  ...
  template: `
    <div class="space-y-6 animate-fade-in-up">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-slate-500 mt-1">
            Take pending quizzes and view upcoming examination schedules.
          </p>
        </div>
      </div>
      ...
      <div *ngIf="!activeExam()" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-4">
          ...
          <div class="glass-panel p-5 relative overflow-hidden group">
            <div
              class="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-red-500/10 to-transparent rounded-bl-full -z-10"
            ></div>
            <div class="flex justify-between items-start mb-4">
              <span
                class="px-2.5 py-1 bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 text-xs font-black tracking-wider rounded-md uppercase"
                >Live Now</span
              >
              <span class="text-xs font-bold text-slate-500">Duration: 30 Mins</span>
            </div>
            <h4 class="font-bold text-slate-900 dark:text-white text-xl leading-tight mb-1">
              CS-101: Midterm Quiz 1
            </h4>
            <p class="text-sm text-slate-500 mb-6">
              Covers Chapters 1-4. Multiple choice questions only.
            </p>

            <button
              (click)="startExam()"
              class="w-full py-3 bg-mit-red hover:bg-primary-600 text-white font-bold rounded-xl shadow-lg shadow-red-500/30 transition-all flex justify-center items-center gap-2"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="18"
          ...
          <div class="glass-panel overflow-hidden">
            <div class="divide-y divide-slate-200 dark:divide-slate-800">
              ...
              >
                <div
                  class="w-14 h-14 rounded-xl bg-slate-100 dark:bg-slate-800 flex flex-col items-center justify-center shrink-0 border border-slate-200 dark:border-slate-700"
                >
          ...
          <div class="glass-card p-6 bg-slate-900 text-white">
            ...
            <ul class="space-y-3 text-sm text-slate-300">
              ...
              <li class="flex gap-2 items-start">
                <div class="w-1.5 h-1.5 rounded-full bg-blue-400 mt-1.5 shrink-0"></div>
                <p>
                  Do not switch browser tabs during live exams. Doing so will trigger an automatic
                  flag.
                </p>
              </li>
              <li class="flex gap-2 items-start">
      ...
      <div *ngIf="activeExam()" class="glass-panel p-8 max-w-4xl mx-auto animate-fade-in">
        <div
          class="flex justify-between items-center mb-8 pb-6 border-b border-slate-200 dark:border-slate-700"
        >
          <div>
          ...
          <div class="flex flex-col items-end">
            <span class="text-xs uppercase font-bold tracking-wider text-slate-400 mb-1"
              >Time Remaining</span
            >
            <div
              class="text-2xl font-mono font-bold"
              [class.text-red-500]="timeRemaining() < 300"
              [class.text-slate-900]="timeRemaining() >= 300"
              [class.dark:text-white]="timeRemaining() >= 300"
            >
              {{ formattedTime() }}
            </div>
          </div>
          ...
          <div class="space-y-3">
            ...
            >
              <input
                ...
                class="w-5 h-5 text-blue-600 border-slate-300 focus:ring-blue-500 dark:bg-slate-800 dark:border-slate-600"
              />
        ...
        <div
          class="flex justify-between items-center pt-6 border-t border-slate-200 dark:border-slate-700"
        >
          <button
            (click)="prevQuestion()"
            [disabled]="currentQuestion() === 0"
            class="px-6 py-2.5 rounded-xl font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors disabled:opacity-50"
          >
            Previous
          </button>
