      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-slate-500 mt-1">
            Select courses for Fall 2026. Minimum 9 credits, Maximum 18 credits.
          </p>
        </div>
        <div class="glass-card px-4 py-2 flex items-center gap-4">
          <div class="flex flex-col">
            ...
            <span
              ...
              [class.text-mit-red]="totalCredits() > 18"
              [class.text-green-600]="totalCredits() >= 9 && totalCredits() <= 18"
              >{{ totalCredits() }}<span class="text-sm text-slate-400">/18</span></span
            >
          </div>
          <button
            [disabled]="totalCredits() < 9 || totalCredits() > 18"
        ...
        <div class="lg:col-span-2 space-y-4">
          <div class="glass-panel p-4 flex justify-between items-center">
            <div class="flex gap-2 overflow-x-auto custom-scrollbar pb-1">
              <button
                class="px-4 py-1.5 rounded-lg bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-sm font-bold whitespace-nowrap"
              >
                Core Subjects
              </button>
              <button
                class="px-4 py-1.5 rounded-lg text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 text-sm font-bold whitespace-nowrap"
              >
                Electives
              </button>
              <button
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div
              *ngFor="let course of availableCourses()"
              class="glass-panel p-5 relative overflow-hidden group"
            >
              <div
                class="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-slate-200/50 dark:from-slate-700/50 to-transparent rounded-bl-full -z-10 group-hover:from-mit-red/10 transition-colors"
              ></div>
              ...
              <div class="flex justify-between items-start mb-2">
                ...
                >
                <span
                  class="text-xs font-bold px-2 py-0.5 bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 rounded"
                  >{{ course.credits }} Credits</span
                >
              </div>

              <h4 class="font-bold text-slate-900 dark:text-white text-lg leading-tight mb-2">
                {{ course.title }}
              </h4>
              ...
              <button
                *ngIf="!isCourseSelected(course.id)"
                (click)="addCourse(course)"
                class="w-full py-2 bg-slate-100 dark:bg-slate-800 hover:bg-mit-red hover:text-white text-slate-700 dark:text-slate-300 font-bold rounded-lg transition-colors text-sm flex justify-center items-center gap-2"
              >
                <svg
              ...
              <button
                *ngIf="isCourseSelected(course.id)"
                ...
                class="w-full py-2 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 border border-green-200 dark:border-green-800/50 font-bold rounded-lg transition-colors text-sm flex justify-center items-center gap-2"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
        ...
        <div class="space-y-4">
          ...
          <div class="glass-panel p-5">
            ...
            <div *ngIf="selectedCourses().length > 0" class="space-y-3">
              <div
                *ngFor="let c of selectedCourses()"
                class="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800"
              >
                <div>
              ...
              <div
                class="border-t border-slate-200 dark:border-slate-700 pt-3 mt-4 flex justify-between items-center font-bold"
              >
                <span class="text-slate-700 dark:text-slate-300">Total Tuition</span>
                <span class="text-slate-900 dark:text-white text-lg"
                  >\${{ totalCredits() * 150 }}</span
                >
              </div>
            </div>
          </div>
          ...
          <div
            class="glass-card p-5 border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-900/10"
          >
            <h4
              class="text-amber-800 dark:text-amber-500 font-bold text-sm flex items-center gap-2 mb-2"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
            ...
            <p class="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
              Your registration is subject to approval by your academic advisor, Dr. Sarah. Ensure
              you meet all prerequisites before submitting.
            </p>
          </div>
        </div>
      </div>
      ...
      next: () => {
        ...
        this.http.post('http://localhost:8080/invoices', invoice).subscribe(
          () => {
            ...
            params.set(
              'return_url',
              window.location.origin + `/student/payment-success?tran_id=${tranId}`,
            );
            window.location.href = `http://localhost:3030/init?${params.toString()}`;
          },
          () => {
            alert(
              'Registration saved but failed to create invoice. Please try payment from Payments page.',
            );
