  ...
  template: `
    <div class="space-y-8 animate-fade-in-up">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Semester Results</h1>
          <p class="text-slate-500 mt-1">Track your detailed marks breakdown, term GPA, and course performance.</p>
        </div>
        <div class="flex items-center gap-3">
          <!-- Semester Selector -->
          <div class="relative">
            <select [(ngModel)]="selectedSemester" (change)="onSemesterChange()" 
          ...
          <!-- Download Button -->
          <button (click)="downloadPdf()" [disabled]="filteredResults().length === 0" 
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="glass-card p-6 border-t-4 border-t-mit-red">
          ...
          <p class="text-[10px] text-slate-400 mt-2 font-semibold uppercase tracking-wider">For {{ selectedSemester() }}</p>
        </div>

        <div class="glass-card p-6 border-t-4 border-t-blue-500">
          <p class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Credits Earned</p>
          <h3 class="text-3xl font-black text-slate-900 dark:text-white">{{ termCredits() }}</h3>
          <p class="text-[10px] text-slate-400 mt-2 font-semibold uppercase tracking-wider">Total Credits Registered</p>
        </div>

        <div class="glass-card p-6 border-t-4 border-t-emerald-500">
          <p class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Passed Courses</p>
          <h3 class="text-3xl font-black text-slate-900 dark:text-white">{{ passedCoursesCount() }} / {{ filteredResults().length }}</h3>
          <p class="text-[10px] text-slate-400 mt-2 font-semibold uppercase tracking-wider">No Failures Recorded</p>
        </div>

        <div class="glass-card p-6 border-t-4 border-t-amber-500">
          <p class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Academic Standing</p>
          ...
          <p class="text-[10px] text-slate-400 mt-2 font-semibold uppercase tracking-wider">Merit Classification</p>
        </div>
      </div>

      <!-- Results Table -->
      <div class="glass-panel overflow-hidden">
        <div class="p-6 border-b border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 flex justify-between items-center">
          <h3 class="font-black text-slate-900 dark:text-white text-lg">Course-wise Marks Breakdown</h3>
          <span class="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-xs font-bold rounded-lg border border-slate-200/50 dark:border-slate-800/50">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/80 text-xs font-bold text-slate-500 uppercase tracking-wider">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
              <tr *ngFor="let r of filteredResults()" class="hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-all duration-200 group">
                <td class="px-6 py-4">
                  <span class="font-mono font-bold text-slate-900 dark:text-white text-sm bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md">{{ r.courseCode }}</span>
                  <div class="text-xs text-slate-500 dark:text-slate-400 font-semibold mt-1.5 group-hover:text-mit-red transition-colors">{{ r.courseTitle }}</div>
                </td>
                <td class="px-6 py-4 text-center text-slate-700 dark:text-slate-300 font-bold">{{ r.credits }}</td>
                <td class="px-6 py-4 text-center text-slate-600 dark:text-slate-400 font-medium">{{ r.midterm ?? '-' }}</td>
                <td class="px-6 py-4 text-center text-slate-600 dark:text-slate-400 font-medium">{{ r.quizzes ?? '-' }}</td>
                <td class="px-6 py-4 text-center text-slate-600 dark:text-slate-400 font-medium">{{ r.assignments ?? '-' }}</td>
                <td class="px-6 py-4 text-center text-slate-600 dark:text-slate-400 font-medium">{{ r.final ?? '-' }}</td>
                <td class="px-6 py-4 text-center">
                  <span class="font-extrabold text-slate-900 dark:text-white">{{ r.total ?? '-' }}</span>
                </td>
                <td class="px-6 py-4 text-center">
                  <span class="px-2.5 py-0.5 text-xs font-black rounded-md border"
                        [ngClass]="{
                          'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-900': !r.grade.startsWith('F'),
                          'bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/20 dark:text-rose-400 dark:border-rose-900': r.grade.startsWith('F')
                        }">
                    {{ r.grade }}
                  </span>
                </td>
                <td class="px-6 py-4 text-center text-slate-700 dark:text-slate-300 font-extrabold">{{ r.gp | number:'1.2-2' }}</td>
                <td class="px-6 py-4 text-center">
              ...
              <tr *ngIf="filteredResults().length === 0">
                <td colspan="10" class="px-6 py-12 text-center text-slate-500 font-semibold italic bg-slate-50/50 dark:bg-slate-900/50">
...
export class StudentResultsComponent implements OnInit {
  ...
  loadData() {
    ...
    this.http.get<any[]>('http://localhost:3000/studentResults').subscribe(data => {
  ...
  downloadPdf() {
    if (this.student() && this.filteredResults().length > 0) {
      this.pdfService.generateSemesterMarksheet(this.student(), this.filteredResults(), this.selectedSemester());
    } else {
