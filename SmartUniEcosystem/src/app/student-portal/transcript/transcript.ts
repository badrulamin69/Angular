  ...
  template: `
    <div class="space-y-6 animate-fade-in-up">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button (click)="downloadPdf()" class="px-5 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl shadow-lg hover:scale-105 transition-transform font-bold flex items-center gap-2">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="glass-panel p-4 text-center">
          ...
          <p class="text-2xl font-black text-slate-900 dark:text-white">{{ student()?.gpa || '3.85' }}</p>
        </div>
        <div class="glass-panel p-4 text-center">
        ...
        <div class="glass-panel overflow-hidden">
          ...
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
              <tr *ngFor="let g of grades()" class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                <td class="px-6 py-3 font-bold text-slate-700 dark:text-slate-300">{{ g.courseId }}</td>
                <td class="px-6 py-3 text-slate-600 dark:text-slate-400">{{ getCourseName(g.courseId) }}</td>
                <td class="px-6 py-3 text-center text-slate-600 dark:text-slate-400">{{ g.credits }}</td>
                <td class="px-6 py-3 text-center font-bold text-green-600" [ngClass]="{'text-blue-600': g.grade.includes('-') || g.grade.includes('B')}">{{ g.grade }}</td>
              </tr>
              <tr *ngIf="grades().length === 0">
                <td colspan="4" class="px-6 py-8 text-center text-slate-500 font-medium">No active academic grades loaded.</td>
              </tr>
            </tbody>
          </table>
        </div>
