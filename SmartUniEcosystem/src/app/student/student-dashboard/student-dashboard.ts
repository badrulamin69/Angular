  ...
  template: `
    <div class="space-y-6 animate-fade-in-up">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <button
          class="px-4 py-2 bg-gradient-to-r from-mit-red to-primary-600 text-white rounded-xl shadow-lg shadow-mit-red/30 hover:shadow-mit-red/50 hover:-translate-y-0.5 transition-all font-bold"
        >
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in delay-100">
        ...
        <div class="glass-card p-6 border-l-4 border-l-indigo-500">
          ...
          <p class="text-xs text-indigo-600 dark:text-indigo-400 mt-2 font-bold">
            Top 15% of Class
          </p>
        </div>
        <div class="glass-card p-6 border-l-4 border-l-emerald-500">
          ...
          <p class="text-xs text-emerald-600 dark:text-emerald-400 mt-2 font-bold">
            Progress to Degree: 65%
          </p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8 animate-fade-in delay-200">
        ...
        <div class="glass-panel p-6">
          <div class="flex justify-between items-center mb-6">
            ...
            <span class="bg-mit-red text-white text-xs font-bold px-2 py-1 rounded-full"
          ...
          <ul class="space-y-4">
            <li
              *ngFor="let n of notifications()"
              class="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            >
              <div class="flex items-start gap-3">
                <div
                  class="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400 shrink-0 mt-0.5"
                >
                  <svg
            ...
            <li *ngIf="notifications().length === 0" class="text-center text-slate-500 py-4">
        ...
        <div class="glass-panel p-6">
          ...
          <div class="space-y-3">
            <div
              *ngFor="let c of courses().slice(0, 3)"
              class="flex items-center justify-between p-3 border-l-4 border-slate-900 dark:border-slate-400 bg-slate-50 dark:bg-slate-800/50 rounded-r-xl"
            >
              <div>
              ...
              <span
                class="text-xs font-bold px-2 py-1 bg-slate-200 dark:bg-slate-700 rounded text-slate-700 dark:text-slate-300"
                >{{ c.credits }} Credits</span
              >
            </div>
            <div *ngIf="courses().length === 0" class="text-center text-slate-500 py-4">
