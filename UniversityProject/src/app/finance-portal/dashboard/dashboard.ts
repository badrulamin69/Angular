    ...
    <div class="space-y-6 animate-fade-in-up">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="glass-card p-6 border-b-4 border-b-emerald-500">
          ...
          <h3 class="text-3xl font-black text-slate-900 dark:text-white">{{ totalRevenue() | currency:'USD':'symbol':'1.0-0' }}</h3>
        ...
        <div class="glass-card p-6 border-b-4 border-b-indigo-500">
          <div class="flex justify-between items-start mb-2">
            ...
            <span class="px-2 py-0.5 bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 text-[10px] font-bold rounded">Due</span>
          </div>
          <h3 class="text-3xl font-black text-slate-900 dark:text-white">{{ outstanding() | currency:'USD':'symbol':'1.0-0' }}</h3>
        ...
        <div class="space-y-4">
          ...
          <div class="glass-panel overflow-hidden">
            <div class="divide-y divide-slate-100 dark:divide-slate-800">
              ...
              <div *ngFor="let invoice of recentInvoices()" class="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                <div class="flex items-center gap-3">
                  <div [ngClass]="invoice.status === 'Paid' ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'" class="w-10 h-10 rounded-full flex items-center justify-center shrink-0">
                  ...
                  <div>
                    <p class="text-sm font-bold text-slate-900 dark:text-white">{{ invoice.studentName || 'Student ID: ' + invoice.studentId }}</p>
                    <p class="text-[10px] text-slate-500 font-mono">#{{ invoice.id }}</p>
                  </div>
                </div>
                <div class="text-right">
                  <p [ngClass]="invoice.status === 'Paid' ? 'text-emerald-500' : 'text-amber-500'" class="text-sm font-black">{{ invoice.amount | currency }}</p>
                  <p class="text-[10px] text-slate-400 font-bold">{{ invoice.dueDate }}</p>
                </div>
              </div>
              
              <div *ngIf="recentInvoices().length === 0" class="p-8 text-center text-slate-500 text-sm">No recent transactions.</div>
