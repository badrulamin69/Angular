    ...
    <div class="space-y-6 animate-fade-in-up">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="glass-card p-6 border-b-4 border-b-violet-500 flex flex-col justify-between">
          <div>
            ...
            <h3 class="text-4xl font-black text-slate-900 dark:text-white">{{ progress()?.totalHoursLearned || 0 }}<span class="text-lg text-slate-400">h</span></h3>
          </div>
        ...
        <div class="glass-card p-6 border-b-4 border-b-fuchsia-500 flex flex-col justify-between">
          <div>
            <p class="text-sm font-bold text-slate-500 uppercase tracking-wider mb-2">Courses Active</p>
        ...
        <div class="glass-card p-6 border-b-4 border-b-amber-500 flex flex-col justify-between">
          <div>
            ...
            <h3 class="text-4xl font-black text-slate-900 dark:text-white">{{ progress()?.certificatesEarned || 0 }}</h3>
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-4">
          ...
          <div *ngIf="lastCourse()" class="glass-panel overflow-hidden flex flex-col md:flex-row group hover:shadow-lg transition-all cursor-pointer">
            <div class="md:w-1/3 bg-gradient-to-br from-violet-600 to-indigo-600 p-6 flex flex-col justify-center relative overflow-hidden">
              ...
              <p class="text-white/80 text-xs font-bold mt-2 relative z-10">{{ lastModule()?.title }}</p>
            </div>
            <div class="p-6 md:w-2/3 flex flex-col justify-between">
              <div>
                ...
                <h5 class="font-bold text-slate-900 dark:text-white text-lg mb-1">Up Next: {{ lastModule()?.title }}</h5>
                <p class="text-sm text-slate-500 line-clamp-2">Resume where you left off. Module type: {{ lastModule()?.type }} ({{ lastModule()?.duration }})</p>
              </div>
              <div class="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                <div class="flex justify-between text-xs mb-1 font-bold">
                  <span class="text-slate-600 dark:text-slate-400">Course Progress</span>
                  <span class="text-violet-600 dark:text-violet-400">{{ lastCourseEnrollment()?.progress }}%</span>
                </div>
                <div class="w-full bg-slate-100 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                  <div class="bg-violet-600 h-full rounded-full" [style.width.%]="lastCourseEnrollment()?.progress"></div>
                </div>
              </div>
            </div>
          </div>
...
export class LmsDashboardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get<any[]>('http://localhost:3000/userProgress').subscribe(data => {
      ...
      if (userProgress) {
        ...
        this.http.get<any[]>('http://localhost:3000/courseContent').subscribe(modules => {
