      ...
      <div class="container mx-auto px-4 md:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-12">
          ...
          <div class="lg:col-span-1 space-y-8">
            <div>
              <h4 class="text-xs font-black uppercase tracking-widest text-slate-500 mb-4">Faculties</h4>
              <div class="space-y-2">
                <button (click)="filterDept('')" class="w-full text-left px-4 py-2 rounded-xl text-sm font-bold transition-all" [class.bg-indigo-600]="!selectedDept()" [class.text-white]="!selectedDept()" [class.hover:bg-slate-100]="selectedDept()">All Programs</button>
                <button *ngFor="let dept of departments()" 
                        (click)="filterDept(dept.id)"
                        class="w-full text-left px-4 py-2 rounded-xl text-sm font-bold transition-all"
                        [class.bg-indigo-600]="selectedDept() === dept.id"
                        [class.text-white]="selectedDept() === dept.id"
                        [class.hover:bg-slate-100]="selectedDept() !== dept.id">
                  {{ dept.name }}
                </button>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div *ngFor="let prog of filteredPrograms()" class="glass-card p-8 group hover:shadow-2xl transition-all border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
                ...
                <div class="flex items-center justify-between pt-6 border-t border-slate-100 dark:border-slate-800">
                  <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">{{ getDeptName(prog.departmentId) }}</span>
...
export class ProgramsListingComponent implements OnInit {
  ...
  filteredPrograms = computed(() => {
    ...
    if (!dept) return this.programs();
    return this.programs().filter(p => p.departmentId === dept);
  });
  ...
  filterDept(id: string) {
    this.selectedDept.set(id);
  }

  getDeptName(id: string) {
    return this.departments().find(d => d.id === id)?.name || 'Academic Faculty';
  }
}
