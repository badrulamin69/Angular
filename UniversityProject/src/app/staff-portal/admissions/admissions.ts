      ...
      <div class="glass-panel overflow-hidden">
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
            <tr *ngFor="let app of applications()" class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
              <td class="px-6 py-4 font-mono text-sm text-slate-600 dark:text-slate-400">{{ app.id }}</td>
              <td class="px-6 py-4">
                <div class="font-bold text-slate-900 dark:text-white leading-tight">{{ app.applicantName }}</div>
                <div class="text-[10px] text-slate-500 uppercase font-bold">{{ app.email }}</div>
              </td>
              <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{{ app.program }}</td>
              <td class="px-6 py-4 font-bold text-slate-900 dark:text-white">{{ app.priorGPA }}</td>
              <td class="px-6 py-4">
                <span [ngClass]="{
                  'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400': app.status === 'New',
                  'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400': app.status === 'In Review',
                  'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400': app.status === 'Accepted',
              ...
              <td class="px-6 py-4 text-right flex justify-end gap-2">
                <button *ngIf="app.status === 'New'" (click)="updateStatus(app.id, 'In Review')" class="px-3 py-1.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-xs font-bold rounded-lg hover:bg-slate-800 dark:hover:bg-slate-200 transition-colors">Start Review</button>
                
                <ng-container *ngIf="app.status === 'In Review'">
                  <button (click)="updateStatus(app.id, 'Accepted')" class="w-8 h-8 rounded-lg bg-teal-50 text-teal-600 dark:bg-teal-900/30 dark:text-teal-400 flex items-center justify-center hover:bg-teal-100 dark:hover:bg-teal-900/50 transition-colors" title="Approve">
                ...
                <button *ngIf="app.status === 'Accepted'" class="text-teal-600 dark:text-teal-400 hover:underline font-bold text-sm">Enroll Now</button>
              </td>
            </tr>
...
export class StaffAdmissionsComponent implements OnInit {
  ...
  stats = computed(() => {
    ...
    return {
      ...
      review: apps.filter(a => a.status === 'In Review').length,
  ...
  loadApplications() {
    this.http.get<any[]>('http://localhost:3000/studentApplications').subscribe(data => {
      this.applications.set(data);
    });
  }

  updateStatus(id: string, status: string) {
    this.http.patch(`http://localhost:3000/studentApplications/${id}`, { status }).subscribe(() => {
